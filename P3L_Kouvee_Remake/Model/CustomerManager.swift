...
struct CustomerManager
{
    ...
    func fetch_all()
    {
        ...
        AF.request(req).response { response in
            ...
            if let data = response.data
            {
                ...
                {
                    self.delegate?.didFetch(customer: safeData)
                }
                else
                {
    ...
    func store_data(nama: String, lahir: String, alamat: String, telp: String, email: String)
    {
    ...
    func parseJson(data: Data) -> Customer?
    {
        let decoder = JSONDecoder()
        
        do
        {
            let decodedData = try decoder.decode(Customer.self, from: data)
            let customerData = decodedData
